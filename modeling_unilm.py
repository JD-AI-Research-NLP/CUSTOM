...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.aspect_embeddings = nn.Embedding(5, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, aspects=None):
        ...
        aspect_embeddings = self.aspect_embeddings(aspects)
        aspect_embeddings = aspect_embeddings.unsqueeze_(1).expand_as(inputs_embeds)

        embeddings = inputs_embeds + position_embeddings + token_type_embeddings + aspect_embeddings
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, sent_probs=None):
        ...
        if sent_probs is not None:
            sent_probs = torch.unsqueeze(sent_probs, 1).repeat(1, 12, 1, 1)
            attention_probs_reweight = attention_probs * sent_probs # 利用sent probs重新归一化
            attention_probs_reweight = attention_probs_reweight / torch.sum(attention_probs_reweight, -1, keepdim=True)
            attention_probs = attention_probs_reweight
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, sent_probs=None):
        self_output = self.self(
            input_tensor, attention_mask, history_states=history_states, sent_probs=sent_probs)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, sent_probs=None):
        attention_output = self.attention(
            hidden_states, attention_mask, history_states=history_states, sent_probs=sent_probs)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, sent_probs=None):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for layer_module in self.layer:
                hidden_states = layer_module(
                    hidden_states, attention_mask, sent_probs=sent_probs)
                if output_all_encoded_layers:
                    all_encoder_layers.append(hidden_states)
...
class UnilmModel(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, aspect=None, sent_probs=None):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)
        embedding_output = self.embeddings(
            input_ids, token_type_ids, aspects=aspect)
        encoded_layers = self.encoder(embedding_output, extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers, sent_probs=sent_probs)
...
class UnilmModelIncr(UnilmModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, aspects=None, sent_probs=None):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)

        embedding_output = self.embeddings(
            input_ids, token_type_ids, position_ids, aspects=aspects)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers,
                                      prev_embedding=prev_embedding,
                                      prev_encoded_layers=prev_encoded_layers, sent_probs=sent_probs)
        sequence_output = encoded_layers[-1]
        pooled_output = self.pooler(sequence_output)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
...
class PredictSentence(nn.Module):
    def __init__(self, config):
        super(PredictSentence, self).__init__()
    ...
    def forward(self, hidden_states, mask):
        ...
        sent_scores = self.sigmoid(self.decoder(hidden_states))
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = UnilmModel(config)
        self.cls = BertOnlyMLMHead(config)
        self.cls_sent = PredictSentence(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')
        self.bce_sent_cls = nn.BCELoss(reduction='none')
        if hasattr(config, 'label_smoothing') and config.label_smoothing:
            self.crit_mask_lm_smoothed = LabelSmoothingLoss(
                config.label_smoothing, config.vocab_size, ignore_index=0, reduction='none')
        else:
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, masked_pos=None, masked_weights=None, num_tokens_a=None, num_tokens_b=None, aspect=None, focus=None, clss=None, mask_clss=None, mask_word_sent_id=None):
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, aspect=aspect)
        ...
        # add sentence classification loss
        sents_vec = sequence_output[torch.arange(sequence_output.size(0)).unsqueeze(1), clss]
        sents_vec = sents_vec * mask_clss[:, :, None].float()
        sent_scores = self.cls_sent(sents_vec, mask_clss)
        sent_cls_loss = self.bce_sent_cls(sent_scores, focus.float())
        sent_cls_loss = (sent_cls_loss * mask_clss.float()).sum() / sent_cls_loss.numel()
        
        # 将句子权重分配到每个词，重新计算上下文
        length_input = torch.sum(attention_mask[:, 0], dim=-1) # 获取输入的长度，不包括target
        sent_probs_projected = torch.gather(sent_scores, 1, mask_word_sent_id) # shape (batch_size, max_len)
        attention_mask_with_sentscore = torch.zeros_like(attention_mask).float()
        for sent_num, sent_leng in enumerate(length_input):
            left_part = (attention_mask[sent_num][:, :sent_leng]).float()
            left_part[sent_leng:, :sent_leng] = sent_probs_projected[sent_num][:sent_leng]
            right_part = (attention_mask[sent_num][:, sent_leng:]).float()
            concat = torch.cat((left_part, right_part), -1)
            attention_mask_with_sentscore[sent_num] = concat
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, aspect=aspect, sent_probs=attention_mask_with_sentscore)
...
#             return prediction_scores
        ...
        return masked_lm_loss, sent_cls_loss
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, input_ids_pad, token_type_ids, position_ids, attention_mask, aspects, clss, mask_cls, mask_word_sent_id):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, input_ids_pad, token_type_ids, position_ids, attention_mask, aspects, clss, mask_cls, mask_word_sent_id)
    ...
    def beam_search(self, input_ids, input_ids_pad, token_type_ids, position_ids, attention_mask, aspects, clss, mask_clss, mask_word_sent_id):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        ...
        # 获取extractor分类结果
        embedding_output, sequence_output, __ = self.bert(input_ids_pad, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=False, aspects=aspects)
        sents_vec = sequence_output[torch.arange(sequence_output.size(0)).unsqueeze(1), clss]
        sents_vec = sents_vec * mask_clss[:, :, None].float()
        sent_scores = self.cls_sent(sents_vec, mask_clss)

        length_input = torch.sum(attention_mask[:, 0], dim=-1)
        sent_probs_projected = torch.gather(sent_scores, 1, mask_word_sent_id) # shape (batch_size, max_a_len)
        while next_pos < output_length:
            curr_length = list(curr_ids.size())[1]
            ...
            # 将句子权重分配到每个词
            if prev_embedding is None:
                attention_mask_with_sentscore = torch.ones(x_input_ids.size()[0], input_ids.size()[1], input_ids.size()[1]+1).to(torch.device("cuda"))
                sent_probs_projected_extend = torch.cat((sent_probs_projected, torch.ones(sent_probs_projected.size()[0], 1).to(torch.device("cuda"))), dim=-1)
                sent_probs_projected_extend = torch.unsqueeze(sent_probs_projected_extend, 1).repeat(1, 1, 1)
                attention_mask_with_sentscore = torch.cat((attention_mask_with_sentscore, sent_probs_projected_extend), dim=1)
            else:
                sent_probs_projected_extend = torch.cat((sent_probs_projected, torch.ones(sent_probs_projected.size()[0], next_pos-input_length+1).to(torch.device("cuda"))), dim=-1)
                attention_mask_with_sentscore = torch.unsqueeze(sent_probs_projected_extend, 1).repeat(1, 2, 1)
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask,
                          output_all_encoded_layers=True, prev_embedding=prev_embedding, prev_encoded_layers=prev_encoded_layers, aspects=aspects, sent_probs=attention_mask_with_sentscore)
            last_hidden = new_encoded_layers[-1][:, -1:, :]
            prediction_scores = self.cls(last_hidden)
            ...
            if is_first:
                ...
                aspects = first_expand(aspects)
                sent_probs_projected = first_expand(sent_probs_projected)
