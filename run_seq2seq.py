...
def main():
    ...
    parser.add_argument("--asp_file", default=None, type=str,
                        help="The output data file name.")
    parser.add_argument("--focus_file", default=None, type=str,
                        help="The output data file name.")
    parser.add_argument("--src_file_dev", default=None, type=str,
                        help="The input data file name.")
    parser.add_argument("--tgt_file_dev", default=None, type=str,
                        help="The output data file name.")
    parser.add_argument("--asp_file_dev", default=None, type=str,
                        help="The output data file name.")
    parser.add_argument("--focus_file_dev", default=None, type=str,
                        help="The output data file name.")
...
#         writer = SummaryWriter('./log/shouji_continue')
    ...
    if args.do_train:
        ...
        fn_aspect = os.path.join(args.data_dir, args.asp_file if args.asp_file else 'train.tgt')
        fn_focus = os.path.join(args.data_dir, args.focus_file)
        train_dataset = utils_seq2seq.Seq2SeqDataset(
            fn_src, fn_tgt, fn_aspect, fn_focus, args.train_batch_size, data_tokenizer, args.max_seq_length, bi_uni_pipeline=bi_uni_pipeline)
        # 定义验证集
        fn_src_dev = os.path.join(
            args.data_dir, args.src_file_dev if args.src_file else 'train.src')
        fn_tgt_dev = os.path.join(
            args.data_dir, args.tgt_file_dev if args.tgt_file else 'train.tgt')
        fn_aspect_dev = os.path.join(args.data_dir, args.asp_file_dev if args.asp_file_dev else 'train.tgt')
        fn_focus_dev = os.path.join(args.data_dir, args.focus_file_dev)

        dev_dataset = utils_seq2seq.Seq2SeqDataset(
            fn_src_dev, fn_tgt_dev, fn_aspect_dev, fn_focus_dev, args.train_batch_size, data_tokenizer, args.max_seq_length, bi_uni_pipeline=bi_uni_pipeline)
        ...
        dev_dataloader = torch.utils.data.DataLoader(dev_dataset, batch_size=_batch_size, sampler=dev_sampler, shuffle=False, num_workers=args.num_workers, collate_fn=utils_seq2seq.batch_list_to_batch_tensors, pin_memory=False)
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                input_ids, segment_ids, input_mask, lm_label_ids, masked_pos, masked_weights, _, aspects, focus, clss, mask_clss, mask_word_sent_id = batch
                masked_lm_loss, sent_cls_loss = model(input_ids, segment_ids, input_mask, lm_label_ids,
                                       masked_pos=masked_pos, masked_weights=masked_weights, aspect=aspects, focus=focus, clss=clss, mask_clss=mask_clss, mask_word_sent_id=mask_word_sent_id)
                if n_gpu > 1:    # mean() to average on multi-gpu.
                    # loss = loss.mean()
                    masked_lm_loss = masked_lm_loss.mean()
                loss = masked_lm_loss + sent_cls_loss
                if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                    writer.add_scalar('train/masked_lm_loss', masked_lm_loss.item(), total_step)
                    writer.add_scalar('train/sent_cls_loss', sent_cls_loss.item(), total_step)
...
#                 break
            ...
            if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                logger.info("** ** * start evaluating epoch %d ** ** * "%i_epoch)
            model.eval()
            total_loss = 0.0
            iter_bar_dev = tqdm(dev_dataloader, desc='Iter (loss=X.XXX)', disable=args.local_rank not in (-1, 0))
            with torch.no_grad():
                for step, batch in enumerate(iter_bar_dev):
...
#                     break
            ...
            dev_loss_reduce = reduce_tensor(total_loss)
            # Save a trained model
            if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                logger.info("** ** * eval loss ** ** * {:.3f}".format(dev_loss_reduce))
                writer.add_scalar('eval/loss', dev_loss_reduce, i_epoch)
                logger.info(
