...
#     url = "http://alpha-writing.jd.local/product/material/image/query?sku="+sku
    try:
      ...
      imgs = get_img_from_html(html)
      csses = re.findall("sku-market-gw\.jd\.com/css.*\?t=\d+", html)
#       import pdb
#       pdb.set_trace()
      for css in csses:
#        import pdb
#        pdb.set_trace()
        tmp_css = 'http://' + css
        html1 = requests.get(tmp_css).text
        for img in get_img_from_html(html1):
          imgs.append(img)
      
      for img in imgs:
        w1.write(sku + '\t' + p_id + '\t' + img + '\n')
        dic[sku] = 1  
...
if __name__ == '__main__':
  ...
  os.system('sh ./ocr.sh' + ' ' + tmp + ' ' + sku_itemid_url_file + ' ' + sku_itemid_ocr_file)
  ...
  handle_ocr_step1(os.path.join(tmp, 'sku_itemid_url'), os.path.join(root, in_folder, dataset + '.img'))
