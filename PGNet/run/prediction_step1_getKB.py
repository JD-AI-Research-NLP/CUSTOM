...
def get_input_info_for_new_sku(new_sku_file, kb, skuBasicInfo):
              ...
              with open(new_sku_file, 'r', encoding='utf-8') as f:
                ...
                for sku in skus:
                  ...
                  if sku in kb and sku in skuBasicInfo: #and skuBasicInfo[sku]['CategoryL3'] in validCat3list:
                    #print(skuBasicInfo[sku]['CategoryL3'])
                    kvs = kb.get(sku, [])
                    if (len(kvs) != 0):
                      index += 1
...
if __name__ == "__main__":
  ...
  if(True): #chuju-test: data_processing_step-1: prepare sku infomation including sku title and KB [For prediction]
    ...
    folder = sys.argv[4]
    cat3_list = sys.argv[5:]
    #print(len(sys.argv))
    #print(cat3_list)
    #cat3_list = ['茶壶', '保温杯', '玻璃杯', '茶杯']
    sku_basic_info_file = os.path.join(root, domain + '_' + folder, domain + '.kb.skuInfo')
    #print(sku_basic_info_file)
    sku_ext_info_file = os.path.join(root, domain + '_' + folder, domain + '.kb.ext.filtered')
    sku_spec_info_file = os.path.join(root, domain + '_' + folder, domain + '.kb.spec.filtered.concat')
    # test_file = r'/home/baojunwei/project/alpha_writing/data/xuanpin2/xuanpin2'
    # sku_basic_info_file = r'D:\Research\Projects\AlphaWriting\model\data\jiayongdianqi_rawData\jiayongdianqi.kb.skuInfo'
    # sku_ext_info_file = r'D:\Research\Projects\AlphaWriting\model\data\jiayongdianqi_rawData\jiayongdianqi.kb.ext.0408.filtered'
    # sku_spec_info_file = r'D:\Research\Projects\AlphaWriting\model\data\jiayongdianqi_rawData\jiayongdianqi.kb.spec.0408.filtered.concat'
    ...
    # test_file = r'D:\Research\Projects\AlphaWriting\model\data\alphaSales\alphaSales'
    test_file = os.path.join(root, domain + '_' + dataset, dataset)
    sku_list = [_.strip('\n').split('\t')[0] for _ in open(test_file, 'r', encoding='utf-8')]
    print(len(sku_list))
    print(sku_list[0])
    print(sku_list[1])
    skuBasicInfo = load_sku_basic_info(sku_basic_info_file, sku_list)
    print('skuBasicInfo: {}'.format(len(skuBasicInfo)))
    kb = {}
    #kb = load_partial_kb(sku_ext_info_file, sku_list, kb)  # jiayongdianqi.kb.ext.0408.filtered
    #kb = load_partial_kb(sku_spec_info_file, sku_list, kb)  # jiayongdianqi.kb.spec.0408.filtered.concat
    ...
    if(False):
      ocr_file = os.path.join(root, dataset, dataset + r'.ocr')
      dumpOCR(test_file, ocr_file)
      ckg = None
      valid_sku_file = os.path.join(root, dataset, dataset + r'.validSku')
      valid_ocr_file = os.path.join(root, dataset, dataset + r'.validOcr')
      sp_file = os.path.join(root, dataset, dataset + r'.sp')
      miningSP(test_file, valid_ocr_file, sp_file, ckg)
