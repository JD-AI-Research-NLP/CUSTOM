...
for line in r2.readlines():
   ...
   sp = ''
   sp =clean_sp(line.strip().split('\t')[-2], dic, all_title)
   area =line.strip().split('\t')[1]
   if sp.find('显示')==len(sp)-2 and len(sp)>3:
      sp = sp+'屏'
      #print(sp)
   if sp.find('循环风量')==0:
      sp = '大'+sp
   ...
   if sp.find('立方大吸力')==0:
      sp = '大吸力' 
   if len(sp)>2:
      skus[line.strip().split('\t')[0]].append(sp+'\t'+str(area))
   #print(skus)
   #if train_text.find(line.strip().split('\t')[-2])>=0:
...
for s in skus:
  ...
  if len(t_list)==0:
     continue
  for str1 in t_list:
    list_sp_area = str1.split('\t')
    del_sku[list_sp_area[0]]=float(list_sp_area[1])
  d1 = sorted(del_sku.items(),key=lambda k:k[1],reverse=True)
  for i in range(min(7,len(d1))):
    tmp_sku[s].append(d1[i][0])
  #print(tmp_sku[s])
skus = tmp_sku
