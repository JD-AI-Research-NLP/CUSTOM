...
if __name__ == "__main__":
  ...
  folder = sys.argv[3]
  if (True):
    id2kb = load_kbInfo(os.path.join(root, domain + '_' + folder, "train" + '.writing.reduced.joinKB'))
    if not os.path.isdir(os.path.join(root, 'ckg')):
      os.mkdir(os.path.join(root, 'ckg'))
    if not os.path.isdir(os.path.join(root, 'ckg', domain)):
      os.mkdir(os.path.join(root, 'ckg', domain))
    if not os.path.isdir(os.path.join(root, 'ckg', domain, 'ckg0')):
      os.mkdir(os.path.join(root, 'ckg', domain, 'ckg0'))

    groupByCategoryL3(id2kb=id2kb, distinct_level='sku', file=os.path.join(root, 'ckg', domain, 'ckg0', 'ckg0.'))
