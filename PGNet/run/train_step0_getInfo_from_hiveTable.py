...
def split_spec_ext(input, out_1, out_2):
  with open(out_1, 'w', encoding='utf-8') as w1:
    with open(out_2, 'w', encoding='utf-8') as w2:
      with open(input, 'r', encoding='utf-8') as f:
        while(True):
          ...
          r1 = parts[3]
          if r1 == 'EXTRA':
            new_line_1 = parts[0] + '\t' + parts[4] + '\t' + parts[5] + '\t' + parts[6]+ \
                         '\t' + parts[7]+ '\t' + parts[8]+ '\t' + parts[9]+ '\t' + parts[10]
            w1.write(new_line_1 + '\n')
          else:
            new_line_2 = parts[0] + '\t' + parts[2] + '\t' + parts[3] + '\t' + parts[4] + '\t' + parts[5] + '\t' + parts[6] + \
                         '\t' + parts[7] + '\t' + parts[8] + '\t' + parts[9] + '\t' + parts[10]
            w2.write(new_line_2 + '\n')


def transfer_skuinfo_format_ori(input, output):
  with open(output, 'w', encoding='utf-8') as w:
    with open(input, 'r', encoding='utf-8') as f:
      while(True):
        ...
        w.write(new_line + '\n')
def transfer_skuinfo_format(input, output):
  with open(output, 'w', encoding='utf-8') as w:
    with open(input, 'r', encoding='utf-8') as f:
      while(True):
        line = f.readline()
...
if __name__ == "__main__":
  ...
  if(True):
    ...
    input_file = os.path.join(domain_path, 'train.kb')
    out_1 = os.path.join(domain_path, domain + '.kb.ext')
    out_2 = os.path.join(domain_path, domain + '.kb.spec')

    split_spec_ext(input_file, out_1, out_2)

    input_file = os.path.join(domain_path, 'train.gdm')
    out = os.path.join(domain_path, domain + '.kb.skuInfo')
    transfer_skuinfo_format(input_file, out)
