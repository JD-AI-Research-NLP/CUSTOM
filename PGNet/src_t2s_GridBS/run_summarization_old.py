...
def setup_training(model, vocab, vocab_aspect, hps,  queue_ = None):
  ...
  try:
    run_training(saver, model, batcher, sess_context_manager, sv, summary_writer, queue_) # this is an infinite loop until interrupted
  except KeyboardInterrupt:
    tf.logging.info("Caught keyboard interrupt on worker. Stopping supervisor...")
    sv.stop()


def run_training(saver, model, batcher, sess_context_manager, sv, summary_writer, queue_):
  ...
  with sess_context_manager as sess:
    ...
    while True: # repeats until interrupted
      if queue_.qsize() > 0:
         tmp = queue_.get()
         if tmp ==1:
            tf.logging.info('decay lr')
            _ = util.load_ckpt(saver, sess, "eval") 
            tf.logging.info('train process load eval best model')
            model.lr = model.lr/2
         elif tmp ==0:
            tf.logging.info('kill train process')
            pid = os.getpid()
            cmd = 'kill ' + str(pid)
            os.system(cmd)         
      ...
      loss = results['loss']
      tf.logging.info('step: %s ; loss: %f', results['global_step'], loss) # print the loss to screen
...
def run_eval(model, vocab, vocab_aspect, hps, queue_=None):
  """Repeatedly runs eval iterations, logging to screen and writing summaries. Saves the model with the best loss seen so far."""
  time.sleep(120)
  batcher = Batcher(FLAGS.eval_data_path, FLAGS.eval_constraint_path, FLAGS.eval_ocr_path, vocab, vocab_aspect, hps, single_pass=FLAGS.single_pass)
  model.build_graph() # build the graph
  saver = tf.train.Saver(max_to_keep=5) # we will keep 3 best checkpoints at a time
  sess = tf.Session(config=util.get_config())
  ...
  while True:
    ...
    if best_loss is None or running_avg_loss < best_loss:
      ...
      if best_step != train_step:
         saver.save(sess, bestmodel_save_path, global_step=train_step, latest_filename='checkpoint')
      best_loss = running_avg_loss
      best_step = train_step
      #halve_time = 0
    if train_step - best_step > FLAGS.steps_halve:
      queue_.put(1)
      halve_time +=1
      #bad_step = train_step
      halve_step = train_step
    if halve_time is not None and halve_time > FLAGS.steps_kill:
      queue_.put(0)
      pid = os.getpid()
      cmd = 'kill ' + str(pid)
      os.system(cmd)
...
#   ckg = CKG(FLAGS.ckg_path)
  ...
  hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm', 'hidden_dim',
                 'emb_dim', 'aspect_dim', 'batch_size', 'beam_size', 'max_dec_steps', 'max_enc_steps', 'max_enc_table_steps', 'coverage', 'cov_loss_wt', 'pointer_gen','arg1','arg2']
  hps_dict = {}
  ...
  if hps.mode == 'train':
    ...
    decoder = BeamSearchDecoder(model, batcher, vocab, ckg, funcWords, black_list, stopWords, consDecoder, bigramlm, confictValues, trigram,trigram_v2,train_text, source_dict, "eval")
