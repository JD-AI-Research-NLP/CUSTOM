...
class ConstraintHypothesis(object):
  ...
  def should_not_be_pruned(self, vocab, batch, ckg, funcWords, black_list, stopWords):
    if self.contain_uniq_bigrams() \
        ...
        and self.consistent(vocab, batch, ckg, funcWords):
      return True
    return False
...
class ConstrainedDecoder(object):
  ...
  def search(self, sess, model, vocab, ckg, funcWords, black_list, stopWords, batch,
             start_hyp, grid_height, enc_states, max_source_len, beam_size):
      ...
      for j in range(j_start, j_end):
        ...
        if (i - 1, j) in search_grid:
          generation_hyps = self.get_generation_hyps(
            ...
            enc_states=enc_states, ckg=ckg, funcWords=funcWords, black_list=black_list, stopWords=stopWords)
        ...
        if (i - 1, j - 1) in search_grid:
          ...
          for hyp in new_constraint_hyps:
            if hyp.should_not_be_pruned(vocab=vocab, batch=batch, ckg=ckg, funcWords=funcWords, black_list=black_list,
                                        stopWords=stopWords):
              new_beam.add(hyp)
  ...
  def get_generation_hyps(self, sess, model, vocab, batch, beam, enc_states, ckg, funcWords, black_list, stopWords):
    ...
    self.dumb_generate(sess=sess, model=model, vocab=vocab, batch=batch, hyp=hyp, enc_states=enc_states,
                       ckg=ckg, funcWords=funcWords, black_list=black_list, stopWords=stopWords, n_best=FLAGS.beam_size)
    for hyp in
    beam if
  ...
  def dumb_generate(self, sess, model, vocab, batch, hyp, enc_states, ckg, funcWords, black_list, stopWords, n_best):
    ...
    while (valid_cnt < n_best and i < 100):
      ...
      if new_hyp.should_not_be_pruned(vocab=vocab, batch=batch, ckg=ckg, funcWords=funcWords, black_list=black_list,
                                      stopWords=stopWords):
...
def run_beam_search(sess, model, vocab, ckg, funcWords, black_list, stopWords, batch, consDecoder, counter):
