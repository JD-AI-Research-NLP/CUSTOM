...
class ConstraintHypothesis(object):
  ...
  def trigram_func(self,vocab,batch,trigram_v2):
    ...
    if len(text)<3:
      ...
      for i in range(to_num):
        ...
        #if '。' in tri:
        #  if text[i]=='。':
        #    tri = ' '.join([text[i+1],text[i+2],text[i+3]])
        #  if text[i+1]=='。':
        #    tri = ' '.join([text[i+2],text[i+3]])
        #  if text[i+2]=='。':
        #    tri = ' '.join([text[i],text[i+1]])
        #  if text[i+3]=='。':
        #    tri = ' '.join([text[i],text[i+1],text[i+2]])
        if tri in trigram:
           cor_num+=1
  ...
  def have_dic(self,vocab, batch, train_text, word_dict):
    pred_word = self.plain_word(vocab, batch)
    txt=train_text
    if len(pred_word)<5 or '，'not in pred_word:
       return True
    else:
      ...
      if tmp_str in word_dict:
        word_tmp1 = word_dict[tmp_str]
        list_sents = '$'.join(word_tmp)
        ...
        while(find_index<len(txt)):
          if txt.find(tmp_str,find_index+1)>=0:
            ...
            end_t = find_index+1
            while(start_t>=0):
              if txt[start_t]!='，' and txt[start_t]!='。':
                start_t-=1
              else:
                break
            T = False
            while(end_t<=len(txt)):
              if txt[end_t]!='。' and  (txt[end_t]!='，' or  T==False):
                if txt[end_t]=='，':
                   T =True
                end_t+=1
              else:
...
class AbstractBeam():
  ...
  def add(self, hyp,vocab,batch,trigram_v2,v1,v2):
    ...
    self.hypotheses = sort_hyps_v4(self.hypotheses,vocab,batch,trigram_v2,v1,v2)
    if len(self.hypotheses) > self.size:
      assert len(self.hypotheses) == self.size + 1
      del self.hypotheses[-1]
...
def run_beam_search(sess, model, vocab, ckg, funcWords, black_list, stopWords, batch, consDecoder, counter, trigram,trigram_v2,train_text, dic, source_dict):
  ...
  final_res = sort_hyps_v4(final_res,vocab,batch,trigram_v2,0,0)
  if (len(final_res) > 0):
