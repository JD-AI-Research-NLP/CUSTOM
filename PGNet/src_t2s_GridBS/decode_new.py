...
class BeamSearchDecoder(object):
  ...
  def decode(self, decode_id_path, eval=False):
    ...
    while True:
      ...
      input_ocr = list()
      for fuck in batch.enc_batch[0]:
        input_ocr.append(self._vocab.id2word(fuck))
      ...
      if flag == '1':
        ...
        if FLAGS.force_decoding:
          t1 = time.time()
          best_hyp = beam_search_force_decode.run_beam_search(
...
#                                              counter)
      if best_hyp != None:
        # 存储attention矩阵
        all_ocr_atten_dists = list()
        for attn in best_hyp.attn_dists:
            all_ocr_atten_dists.append(attn[:len(input_ocr)])
        #for j in best_hyp:
        j = best_hyp
...
#           decoded_output = "NULL"
        ...
        if FLAGS.single_pass:
          self.write_for_rouge(original_abstract_sents, decoded_words,j.log_probs,
                           counter, input_ocr, all_ocr_atten_dists)  # write ref summary and decoded summary to file, to eval with pyrouge later
        else:
...
#       break
  ...
  def write_for_rouge(self, reference_sents, decoded_words,log_probs,ex_index, input_ocr=None, all_ocr_atten_dists=None):
    ...
    atten_file = os.path.join(self._rouge_dec_dir, "atten.txt")
    ocr_file = os.path.join(self._rouge_dec_dir, "ocr.txt")
    ...
    with open(decoded_file, "a") as f:
      for idx, sent in enumerate(decoded_sents):
        ...
        f.write(sent + "\n") if idx == len(decoded_sents) - 1 else f.write(sent)
    if input_ocr == None:
      with open(atten_file, "a") as f:
        f.write( 'NULL'+ '\n')
      with open(ocr_file, "a") as f:
        f.write( 'NULL' + '\n')
    else:
      with open(atten_file, "a") as f:
        f.write(str(all_ocr_atten_dists) + '\n')
      with open(ocr_file, "a") as f:
        f.write(' '.join(input_ocr) + '\n')
