...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, enc_aspect, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, selector_probs=None, enc_sent_id_mask=None):
  """
  Args:
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        def masked_attention(e):
          ...
          attn_dist_norescale = attn_dist * enc_padding_mask # apply mask        
      ...
      return context_vector, attn_dist, attn_dist_norescale, coverage
    ...
    if initial_state_attention: # true in decode mode
      ...
      context_vector, _, _, coverage = attention(initial_state, coverage) # in decode mode, this is what updates the coverage vector
...
#       x = linear([inp] + [context_vector] + [enc_aspect], input_size, True)
      x = linear([inp] + [context_vector] + [enc_aspect], input_size, True)
      ...
      if i == 0 and initial_state_attention:  # always true in decode mode
        with variable_scope.variable_scope(variable_scope.get_variable_scope(), reuse=True): # you need this because you've already run the initial attention(...) call
          context_vector, attn_dist, attn_dist_norescale, _ = attention(state, coverage) # don't allow coverage to update
      else:
        context_vector, attn_dist, attn_dist_norescale, coverage = attention(state, coverage)
    ...
    return outputs, state, attn_dists, attn_dists_norescale, p_gens, coverage
