...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file_src, file_tgt, file_aspect, file_focus, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[]):
        super().__init__()
        ...
        self.ex_list = []
        with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt, open(file_aspect, "r", encoding='utf-8') as f_asp, open(file_focus, 'r', encoding='utf-8') as f_focus:
            for src, tgt, asp, focus in zip(f_src, f_tgt, f_asp, f_focus):
                src_ocr = src.split('##')[0].split('$$')[1]
                src_tk = tokenizer.tokenize(src_ocr.strip() + '。')
                tgt_tk = tokenizer.tokenize(tgt.strip())
                asp_tk = int(asp.strip())
                focus_tk = focus.strip()
                assert len(src_tk) > 0
                assert len(tgt_tk) > 0
                self.ex_list.append((src_tk, tgt_tk, asp_tk, focus_tk))
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        ...
        tokens_a, tokens_b, aspect, focus = instance[:4]
...
#         tokens_b = self.tokenizer.tokenize(tokens_b)
        ...
        if tokens_a[-1] != '。':
            tokens_a[-1] = '。'
        clss = [index+1 for index, data in enumerate(tokens_a) if data=='。'] # 记录每个CLS的位置
        if len(clss) > 50:
            clss = clss[:50]
            tokens_a = tokens_a[:clss[-2]+1]
        final_focus = list(map(int, focus.split()[:len(clss)]))
        mask_word_sent_id = list()
        last_index = -1
        for count, quote_index in enumerate(clss):
            sent_start = last_index + 1
            sent_end = quote_index
            for word_num in range(sent_start, sent_end+1):
                mask_word_sent_id.append(count)
            last_index = quote_index 
        mask_word_sent_id.append(mask_word_sent_id[-1])
        # Add Special Tokens
        ...
        n_pad_word_sent = self.max_len - len(mask_word_sent_id)
        mask_word_sent_id.extend([0]*n_pad_word_sent)
        n_pad_word_sent = self.max_len - len(mask_word_sent_id)
        # 获取句子级别的mask，最大句子数量为50
        n_pad_sent = 50 - len(clss)
        final_focus.extend([0] * n_pad_sent)
        clss.extend([-1] * n_pad_sent)
        clss = torch.LongTensor(clss)
        mask_cls = (~(clss == -1)).type(torch.uint8)
        clss[clss == -1] = 0
        ...
        return (input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, next_sentence_label, aspect, final_focus, clss, mask_cls, mask_word_sent_id)
...
class Preprocess4Seq2seqDecode(Pipeline):
    ...
    def __call__(self, instance):
        tokens_a, max_a_len, asp = instance
        ...
        clss = [index for index, data in enumerate(padded_tokens_a) if data=='。'] # 记录每个句号的位置
        if len(clss) > 50:
            clss = clss[:50]
            tokens_a = tokens_a[:clss[-2]+1]
        mask_word_sent_id = list()
        ...
        # Token Indexing
        input_ids = self.indexer(tokens)
        n_pad = max_len_in_batch - len(input_ids)
        input_ids_pad = copy.deepcopy(input_ids)
        input_ids_pad.extend([0]*n_pad)
        mask_word_sent_id.extend([0]*(len(tokens)-len(mask_word_sent_id)))
        # Zero Padding
        ...
        return (input_ids, input_ids_pad, segment_ids, position_ids, input_mask, asp, clss, mask_cls, mask_word_sent_id)
